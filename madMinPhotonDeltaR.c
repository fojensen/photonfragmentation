...
void madMinPhotonDeltaR()
{
   ...
   chain_GJets_DR0p4->Add(dir + "tree_GJets_DR-0p4_HT-100to200_MC2017.root");
   chain_GJets_DR0p4->Add(dir + "tree_GJets_DR-0p4_HT-200to400_MC2017.root");
   chain_GJets_DR0p4->Add(dir + "tree_GJets_DR-0p4_HT-400to600_MC2017.root");
   chain_GJets_DR0p4->Add(dir + "tree_GJets_DR-0p4_HT-600toInf_MC2017.root");
   ...
   const TCut cut_photon = "Sum$(Photons_hasPixelSeed==0 && Photons_fullID && Photons.Pt()>200.)==1";
   const TCut cut_leptons = "NElectrons==0 && NMuons==0"; 
   ...
   sprintf(cut_ZJetsToNuNu, "Weight * 41860. * (%s)", TString(cut_baseline).Data());
   std::cout << "filling from ZJetsToNuNu..." << std::endl;
   chain_ZJetsToNuNu->Project("h_ZJetsToNuNu", "madMinPhotonDeltaR", cut_ZJetsToNuNu);

   char cut_GJets[2000];
   sprintf(cut_GJets, "Weight * 41860. * (%s)", TString(cut_baseline && cut_photon).Data());
   ...
   char cut_QCD[2000];
   sprintf(cut_QCD, "Weight * 41860. * (%s && madMinDeltaRStatus==1)", TString(cut_baseline && cut_photon).Data());
